-- ============================================================
-- SCRIPT DE BASE DE DATOS: MOTOR DE COINCIDENCIAS (MATCHES)
-- ============================================================
...
BEGIN
    ...
    INSERT INTO public.humanitarian_reports (tenant_id, reporter_phone, person_name, age, gender, location, status, companion_info)
    VALUES (p_tenant_id, p_reporter_phone, p_person_name, p_age, p_gender, p_location, p_status, p_companion_info)
    RETURNING id INTO v_new_report_id;

    -- Buscar coincidencias con reportes de ESTADO OPUESTO usando pg_trgm (similitud > 0.4)
    -- Si el estado es "Buscando", cruzamos con "Encontrado" o "Seguro"
    -- Si el estado es "Encontrado" o "Seguro", cruzamos con "Buscando"
