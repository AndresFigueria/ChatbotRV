-- ============================================================
-- 🚀 REPARACIÓN DEFINITIVA DE DATOS CRUZADOS (V3)
-- ============================================================
...
-- 2. Limpieza de duplicados y migración inteligente
DO $$
DECLARE
...
BEGIN
    ...
    -- Iterar sobre todos los chats que tienen tenant_id NULL (los huérfanos)
    FOR r_bad_chat IN 
        SELECT id, phone FROM public.whatsapp_chats WHERE tenant_id IS NULL
    LOOP
        -- Buscar si ya existe un chat del mismo cliente en el tenant activo
        ...
        IF v_good_chat_id IS NOT NULL THEN
            -- CASO DUPLICADO: Ya existe un chat bueno. 
            -- 1. Movemos todos los mensajes del chat malo al chat bueno y les ponemos el tenant_id activo
            UPDATE public.whatsapp_messages 
            SET chat_id = v_good_chat_id, tenant_id = v_active_tenant 
            WHERE chat_id = r_bad_chat.id;

            -- 2. Eliminamos el chat huérfano duplicado
            DELETE FROM public.whatsapp_chats WHERE id = r_bad_chat.id;
        ELSE
            -- CASO ÚNICO: No existe chat en el activo, simplemente actualizamos este chat huérfano
            UPDATE public.whatsapp_chats 
            SET tenant_id = v_active_tenant 
            WHERE id = r_bad_chat.id;
        END IF;
    END LOOP;

    -- Actualizar cualquier mensaje huérfano suelto que haya quedado
    UPDATE public.whatsapp_messages SET tenant_id = v_active_tenant WHERE tenant_id IS NULL;
    
    -- Actualizar clientes huérfanos
