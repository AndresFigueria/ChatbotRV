...
try {
  ...
  const stmt = db.prepare("SELECT data FROM execution_data WHERE executionId = 476");
  const row = stmt.get();
  ...
  if (row && row.data) {
    ...
    if (runData) {
      console.log("=== EXECUTED NODES IN 476 ===");
      Object.keys(runData).forEach(nodeName => {
        const run = runData[nodeName][0];
        console.log(`- Node: ${nodeName} | Status: ${run.executionStatus}`);
      });
    ...
    console.log("Execution 476 data not found.");
  }
} catch (err) {
  console.error(err);
}
