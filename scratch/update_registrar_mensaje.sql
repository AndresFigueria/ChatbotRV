...
BEGIN
  ...
  -- Upsert del chat (conflicto en (phone, tenant_id) para actualizar el chat existente)
  INSERT INTO public.whatsapp_chats
    (customer_id, phone, contact_name, last_message, last_message_at, tenant_id, unread_count)
  VALUES
  ...
  ON CONFLICT (phone, tenant_id) DO UPDATE
    SET last_message    = EXCLUDED.last_message,
        ...
        unread_count    = CASE WHEN p_direction = 'inbound' THEN COALESCE(whatsapp_chats.unread_count, 0) + 1 ELSE COALESCE(whatsapp_chats.unread_count, 0) END
