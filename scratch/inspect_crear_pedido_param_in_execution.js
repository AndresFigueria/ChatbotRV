...
try {
  ...
  const stmt = db.prepare("SELECT data FROM execution_data WHERE executionId = 453");
  const row = stmt.get();
  
  if (row && row.data) {
    const parsed = flattedParse(row.data);
    const runData = parsed.resultData?.runData || parsed[0]?.resultData?.runData;
    const run = runData?.['AI Agent']?.[0] || runData?.['Crear Pedido']?.[0];
    if (run && run.error && run.error.node) {
      console.log("=== CREAR PEDIDO PARAMETERS IN RUN 453 ===");
      console.log("JSON Body:", run.error.node.parameters?.jsonBody);
    } else {
      console.log("Could not find Crear Pedido error node parameters.");
    }
  } else {
    console.log("Execution 453 data not found.");
  }
} catch (err) {
  console.error(err);
}
