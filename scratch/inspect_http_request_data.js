...
try {
  ...
  const stmt = db.prepare("SELECT data FROM execution_data WHERE executionId = 462");
  const row = stmt.get();
  ...
  if (row && row.data) {
    ...
    if (runData && runData['HTTP Request']) {
      console.log("=== HTTP REQUEST OUTPUT IN 462 ===");
      console.log(JSON.stringify(runData['HTTP Request'][0].data.main[0][0].json, null, 2));
    } else {
      console.log("No HTTP Request output found.");
    }
  } else {
    console.log("Execution 462 data not found.");
  }
} catch (err) {
  console.error(err);
}
