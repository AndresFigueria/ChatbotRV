...
try {
  ...
  if (row && row.data) {
    ...
    // Find Crear Pedido node input/output in the execution flow
    const runData = parsed.resultData?.runData || parsed[0]?.resultData?.runData;
    console.log("=== ALL RUN DATA KEYS ===");
    console.log(Object.keys(runData));
    
    console.log("=== CREAR PEDIDO RUN DATA ===");
    console.log(JSON.stringify(runData['Crear Pedido'], null, 2));
  }
} catch (err) {
  console.error(err);
}
