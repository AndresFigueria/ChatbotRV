...
async function run() {
  const { data, error } = await supabase.rpc('get_function_def', { function_name: 'crear_pedido' });
  if (error) {
    console.error("Error calling get_function_def:", error);
    // Let's run a generic query if possible, or try querying directly.
    return;
  }
  console.log("=== CREAR_PEDIDO DEFINITION IN DATABASE ===");
  console.log(data);
}
